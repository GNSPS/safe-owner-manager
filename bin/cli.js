...
const args = minimist(process.argv.slice(2), {
  string: ['safe-address', 'new-owners', 'chain-id', 'new-threshold', 'alchemy-api-key'],
});
...
if (!safeAddress || !newOwnersInput || !chainId || !alchemyApiKey) {
  console.log('Usage: gnosis-safe-owner-manager --safe-address=SAFE_ADDRESS --new-owners=NEW_OWNER_1,NEW_OWNER_2,... --chain-id=CHAIN_ID [--new-threshold=NEW_THRESHOLD] --alchemy-api-key=ALCHEMY_API_KEY');
  process.exit(1);
}
...
generateTransactions({
  ...
  .then(jsonOutput => {
    // Output JSON to file
    fs.writeFileSync('transactions.json', JSON.stringify(jsonOutput, null, 2));
    console.log('Transaction JSON has been saved to transactions.json');
  })
  .catch(error => {
